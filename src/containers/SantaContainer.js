...
export default class SantaContainer extends React.Component {
  constructor(props) {
    super(props);
    ...
    // Tracks when the space bar is clicked so as to make the santa fly
  ...
  handleKeyPress(e) {
    ...
    if (e.keyCode === 32 && !this.props.isGameOver) {
      ...
      let santaHeight = $("#santa").position().top;
      if (santaHeight > 40) {
        // santa is below roof
        $("#santa").animate(
          {
            top: "-=50px", // Subtract from the top on the click of spacebar to elevate the santa
          },
        ...
        self.props.handleGameOver(true); // The santa touched the roof, game over!
        this.isSpaceBarClicked = false;
      }
    }
  }
  // Function that keeps the santa falling
  fallDown() {
    let self = this;
    if (!this.isSpaceBarClicked) {
      let santaHeight = $("#santa").position().top; // Current height of the santa
      if (santaHeight < this.height) {
        // Santa is above the ground
        $("#santa").animate(
          {
            top: "+=25px", // Add to the current 'top' attribute value of the santa
          },
        ...
        // this.isGameOver = true;
        self.props.handleGameOver(true); // The santa touched the ground, game over!
        clearInterval(this.intervalID); // Stop the timer set for 'fallDown()' calling
      }
    }
  }

  collisionOccurred(santa, upperDiv, lowerDiv) {
    // width
    var x1 = santa.offset().left;
    var w1 = santa.width();
    // height
    var y1 = santa.offset().top;
    var h1 = santa.height();
    ...
    if (
      (y1 > finalHeight2 || finalWidth < x2 || x1 > finalWidth2) &&
      (finalHeight < finalHeight3 ||
        y1 < finalHeight3 ||
        finalWidth < x3 ||
        x1 > finalWidth3)
    )
